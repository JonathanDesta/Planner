  ...
  const runner =
    run && !run.finishedAt
      ...
      : run?.finishedAt
        ...
        : `<section class="card"><h2>${Math.round(total / 60)} minutes, with breathing room.</h2><p>${weekday(selectedDate) === 4 ? "Thursday hair care and two-pass face shaving after drying off, before skincare." : weekday(selectedDate) === 2 ? "Your everyday shower, then two-pass shaving after drying off and before skincare." : groomingSunday(selectedDate) ? "Body shaving, eyebrows, nails and the weekly vacuum." : weekday(selectedDate) === 0 ? "Your everyday routine, then the weekly vacuum." : "Your everyday shower and morning routine."}</p><button class="primary" data-action="routine-start" ${selectedDate !== dateISO() ? "disabled" : ""}>Start morning routine</button>${selectedDate !== dateISO() ? `<p class="fine-print">Switch to today to start the live routine.</p>` : ""}</section>`;
  return (
    heading(
      "ONE STEP AT A TIME",
